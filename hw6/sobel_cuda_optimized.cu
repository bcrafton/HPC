...
__global__ void sobel(int* input_image, int* output_image, int image_width, int image_height, int thresh)
{
	__shared__ int shared[(32+2)*(32+2)];	
	...
	int global_offset = global_i*image_width + global_j;
	int offset = i*image_width + j;

	if( (global_i < (image_height-1)) && (global_j < (image_width-1)) )
	{
		shared[(i-1)*BLOCK_SIZE_X + (j-1)] = input_image[(global_i-1)*image_width + (global_j-1)];
		shared[(i-1)*BLOCK_SIZE_X + (j)] = input_image[(global_i-1)*image_width + (global_j)];
		shared[(i-1)*BLOCK_SIZE_X + (j+1)] = input_image[(global_i-1)*image_width + (global_j+1)];
		shared[(i)*BLOCK_SIZE_X + (j-1)] = input_image[(global_i)*image_width + (global_j-1)];
		shared[(i)*BLOCK_SIZE_X + (j)] = input_image[(global_i)*image_width + (global_j)];
		shared[(i)*BLOCK_SIZE_X + (j+1)] = input_image[(global_i)*image_width + (global_j+1)];
		shared[(i+1)*BLOCK_SIZE_X + (j-1)] = input_image[(global_i+1)*image_width + (global_j-1)];		
		shared[(i+1)*BLOCK_SIZE_X + (j)] = input_image[(global_i+1)*image_width + (global_j)];
		shared[(i+1)*BLOCK_SIZE_X + (j+1)] = input_image[(global_i+1)*image_width + (global_j+1)];
		...
		sum1 =  shared[ BLOCK_SIZE_X * (i-1) + j+1 ] -     shared[ BLOCK_SIZE_X*(i-1) + j-1 ]
		+ 2 * shared[ BLOCK_SIZE_X * (i)   + j+1 ] - 2 * shared[ BLOCK_SIZE_X*(i)   + j-1 ]
		+     shared[ BLOCK_SIZE_X * (i+1) + j+1 ] -     shared[ BLOCK_SIZE_X*(i+1) + j-1 ];

		sum2 = shared[ BLOCK_SIZE_X * (i-1) + j-1 ] + 2 * shared[ BLOCK_SIZE_X * (i-1) + j ]  + shared[ BLOCK_SIZE_X * (i-1) + j+1 ]
		- shared[BLOCK_SIZE_X * (i+1) + j-1 ] - 2 * shared[ BLOCK_SIZE_X * (i+1) + j ] - shared[ BLOCK_SIZE_X * (i+1) + j+1 ];

		magnitude =  sum1*sum1 + sum2*sum2;

		if (magnitude > thresh)
			output_image[global_offset] = 255;
		else
			output_image[global_offset] = 0;
	}
}
